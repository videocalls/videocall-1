    ...
    public Map<String, Object> selectMypSvcApplList(MypSvcApplVO mypSvcApplVO) throws Exception{
    	...
    	for(int i=0; i<resultList.size(); i++){
            ...
            if(!OppfStringUtil.isEmpty(termsRegNo)){
                String termsRegNoEncryption = OppfStringUtil.base64Incoding(termsRegNo);
                resultList.get(i).setTermsRegNoEncryption(termsRegNoEncryption);
				//동의서 번호에 따른 금투사 목록 조회
				List<MypSvcApplVO> list = mypSvcApplDAO.selectMypSvcApplCompanyList(resultList.get(i));
				//동의서 번호에 따른 연결된 앱 목록 조회
				List<String> apps = mypSvcApplDAO.selectMypSvcApplAppList(resultList.get(i));
				resultList.get(i).setAppList(apps);
				map.put(termsRegNo, list);
            }
        }
...
//    	List<MypSvcApplVO> resultList = mypSvcApplDAO.selectFintechSvcDiscardList(mypSvcApplVO);
		List<MypSvcApplVO> resultList = mypSvcApplDAO.selectFintechSvcDiscardListNew(mypSvcApplVO);
		map.put("resultList", resultList);
    	...
    	for(int i=0; i<resultList.size(); i++){
            ...
            if(!OppfStringUtil.isEmpty(termsRegNo)){
				...
				//동의서 번호에 따른 금투사 목록 조회
//				List<MypSvcApplVO> list = mypSvcApplDAO.selectMypSvcApplCompanyList(resultList.get(i));
				List<MypSvcApplVO> list = mypSvcApplDAO.selectMypSvcApplCompanyListForDiscard(resultList.get(i));
				map.put(termsRegNo, list);
            }
        }
