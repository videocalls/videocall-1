...
<%--
/**  
 * @Name : fintechSvcChoic.jsp
 * @Description : [핀테크서비스신청:핀테크서비스선택]
...
function fn_login(){
    ...
    <c:if test="${!empty paramVO.appId}">
    var url = "<c:url value='/usr/svcAppl/fintechSvcChoic.do'/>?appId=<c:out value='${paramVO.appId}'/>";
    </c:if>
...
function fn_svcTermsAuthCheck(){
    ...
    if(!g_svcTermsAuthFlag && checkFlag){
        $(".finish").append('<li class="svcTerms" style="background: none;padding-left: 0px;"><input type="checkbox" name="infoTerms" id="infoTerms" checked > <label for="infoTerms" class="chk_box">선택된 핀테크기업에 대한 금융투자사 정보 제공 동의가 완료되어 해당 단계를 건너뜁니다.</label>  </li>');
    }
}
<%-- 검색 --%>
function fn_search(){
    ...
    if(util_chkReturn(resultSvcList, "s") != ""){
        $.each(resultSvcList, function(idx, list){
            ...
            if(list.useYn == "Y"){
            	...
            	html += "<a href='javascript:fn_cancelService(\""+list.serviceRegNo+"\", \""+list.termsAuthYn+"\");' class='"+className+"'>"+serviceCancelMsg+"</a>";
            }
            ...
            <article id="content">
                ...
                <div class="con">
                    ...
                    <div class="title_step step1">
                        ...
                        <p>금융투자회사 선택<span>발급받으신 가상계좌를 기반으로 신청이 가능한 핀테크 서비스를 조회합니다.</span></p>
                    </div>
                    ...
                    <div class="chk_list_box">
                        <ul>
	                        ...
	                        <c:forEach items="${companyCodeList}" var="companyCodeList" varStatus="status">
	                            <li>
	                            <input type="checkbox" name="searchCompanyCodeId" id="searchCompanyCodeId_${companyCodeList.companyProfileRegNo}" value="${companyCodeList.companyCodeId}">
	                            <label for="searchCompanyCodeId_${companyCodeList.companyProfileRegNo}">${companyCodeList.companyNameKorAlias}</label>
	                            </li>
	                        </c:forEach>
                        </ul>
                    </div>
                    ...
                    <div class="title_step step2">
                        ...
                        <p>핀테크 서비스 선택<span>[주요 서비스] 항목에 마우스를 오버하시면 금융투자회사별 연계 서비스를 확인할 수 있습니다.</span></p>
                        <div class="search_box right">
                            <span class="sel_box1">
                                <select style="min-width:;" id="searchUseYn" name="searchUseYn" title="사용여부 선택">
                    ...
                    <!-- 2016-06-02 수정 -->
                    <!-- tbtype_list -->
                    <table class="tbtype_list2" style="">
