...
//         util_movePage("<c:url value='/cpt/cmm/selectIdFindView.do'/>", "?tabId=id");
        fn_moveHttpUrl('/cpt/cmm/selectIdFindView.do?tabId=id');
    });
    
    //패스워드 찾기
    $("#btnSchPw").bind("click", function(){
//         util_movePage("<c:url value='/cpt/cmm/selectPwFindView.do'/>", "?tabId=pw");
        fn_moveHttpUrl('/cpt/cmm/selectPwFindView.do?tabId=pw');
    });
...
function fn_login(){
	...
	if(gfn_nvl(spw) == ""){
 	...
 	var url = "<c:url value='/cpt/cmm/loginCheck.do'/>";
	var reqData = {
       "operator_id"   : $("#loginId").val()
      ,"operator_password" : spw
	};
...
function fn_loginCheck_callBack(data){
	...
	if(loginVO == null){
        ...
        if(<spring:message code='Globals.user.policy.password.failCnt'/> <= loginVO.operator_password_fail_cnt){
        	if(loginVO.operator_reg_status == "G005003"){
	            ...
	            if(loginVO.operator_password_yn != "Y"){
	                var errMsg = "비밀번호가 틀렸습니다.\n";
	                errMsg += "비밀번호 실패 횟수가 "+loginVO.operator_password_fail_cnt+"회 입니다.\n\n";
        ...
        if(loginVO.operator_password_yn == "Y"){
            ...
            <c:choose>
                <c:when test="${empty paramVO.emailSendType}" >
                    ...
                    //미활성 상태
                    if(loginVO.operator_reg_status == "G005001"){
                        alert("회원상태가 미활성화 상태입니다.\n관리자에게 문의 해 주세요.");
                        return;
                        
                    //정지 상태
                    }else if(loginVO.operator_reg_status == "G005003"){
                        alert("회원상태가 정지 상태입니다.\n관리자에게 문의 해 주세요.");
                        return;
                        
                    //탈퇴 상태
                    }else if(loginVO.operator_reg_status == "G005004"){
            ...
            if(loginVO.operator_final_password_yn == "Y" || loginVO.operator_expire_password_yn == "Y" || loginVO.operator_temp_password_yn == "Y"){
                //비밀번호변경 팝업창 오픈                
                var url = "<c:url value='/cpt/myp/cptMyInfo/cptMyPwMod.do'/>";
                
                var objOption = new Object();
                objOption.type = 'modal';
                if(loginVO.operator_final_password_yn == "Y" || loginVO.operator_temp_password_yn == "Y") {
                    objOption.width = '620'; 
                ...
                if(util_chkReturn($("#returnListURL").val(), "s") != "") {
                    ...
                    //메인페이지로 이동
                    var uri = httpContextpath+'/cpt/cmm/mainView.do';
...
function fn_pwChgCallBack(){
	util_movePage("<c:url value='/cpt/cmm/loginView.do'/>");
}
...
<div class="wrap">
    ...
    <section>
        ...
        <div class="location">
            <ul>
                <li class="home"><a href="http://<spring:message code='Globals.domain.cpt' />">홈</a></li>
                <li class="on">로그인</li>
            </ul>
        </div>
        <!-- // location -->
            ...
            <article id="content">
                ...
                <div class="con">
                    ...
                    <div class="member_area">
                        <div class="tit_info">
                            <strong><spring:message code='Globals.domain.cpt.name' />에 오신 것을 환영합니다.</strong>
                        </div>
                        <p class="txt"><spring:message code='Globals.domain.cpt.name' />의 회원이 되시면 코스콤과 핀테크 기업이 제공하는<br>다양한 정보와 증권용 핀테크 서비스를 이용하실 수 있습니다.</p>
                            ...
                            <ul>
                                <li>
                                    ...
                                    <input type="text" name="loginId" id="loginId" class="input_txt01" tabindex="1"
                                           ...
                                           autocomplete="off" e2e_type="1" onkeyup="onMoveFocus(this,'loginId',15);"
                                           value=""
                                    >
                                </li>
                                <li>
                                    ...
                                    <input type="password" name="loginPassword" id="loginPassword" class="input_txt01" maxlength="15" tabindex="2" 
                                     ...
                                     autocomplete="off" e2e_type="1" onkeyup="onMoveFocus(this,'btnLogin',15);"
                                    />
                                </li>
                            </ul>
