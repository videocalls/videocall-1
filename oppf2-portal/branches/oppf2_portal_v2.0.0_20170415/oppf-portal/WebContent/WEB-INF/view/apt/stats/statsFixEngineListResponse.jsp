...
// tab_menu 
   ...
   $("#"+"tab_02").parent().addClass("on");
	...
	g_gridColumnAll = [
		{ text: '일', datafield: 'resultDt', width: '200px', cellsalign: 'center', align: 'center', pinned: true },
		{ text: '전체', datafield: 'totCnt', width: '200px', cellsalign: 'right', align: 'center' }
		
	];
	//초기화
	$("#btnSearchClear").click(function(){
	    util_moveRequest("StatsTrafficVO", "<c:url value='/apt/stats/statsFixEngineReponseList.do'/>");
	});
    ...
    if("<c:out value='${paramVO.searchTrafficType}'/>" != ""){
        ...
        $("#searchTimeTo").val("<c:out value='${PparamVO.searchTimeTo}'/>");	
...
function fn_initiatorAll(){
	fix_selectCodeChkAll(g_initCheckboxArr[2]);
...
function fn_search(){
    ...
    $("input[name=searchInitiatorList]:checked").each(function() {
        var code = $(this).val();
        var value = $(this).next('label').text();
        initiatorList[initiatorList.length] = code + "/" + value;
    });
    $("#initiatorList").val(initiatorList);
    ...
    //page setting  
    var url = "<c:url value='/apt/stats/statsFixEngineResponseajax.do'/>";
...
*/
    ...
    for(var i=0; i<headers.length; i++) {
        if(isAllData) {
            columns.push({ text: '전체', datafield: 'resultCnt', width: '200px', cellsalign: 'right', align: 'center' });

        } else {
            columns.push({ text: headers[i].displayText, datafield: 'resultCnt_'+ headers[i].dataField, width: '200px', cellsalign: 'right', align: 'center' });
    ...
    g_gridColumnAll = [
    	{ text: '일', datafield: 'resultDt', width: '200px', cellsalign: 'center', align: 'center', pinned: true },
    	{ text: '전체', datafield: 'totCnt', width: '700px', cellsalign: 'right', align: 'center' }
 ...
 function fn_searchExcel(excelType){
     ...
     util_moveRequest("StatsTrafficVO", "<c:url value='/apt/stats/statsFixEngineResponseajaxExcel.do'/>", "EXCEL_FRAME");
...
<input type="hidden" name="searchInitiatorListAllYn" id="searchInitiatorListAllYn" value="N" />
...
<input type="hidden" name="initiatorList" id="initiatorList" value="" />
					...
					<div class="tb_write1">
						<table>
							...
							<tr>
								...
								<td class="txt_l" colspan="3"> 
									...
									<div class="chk_list_wrap type2">
						                    <ul>
						                        <li>
						                            <input type="checkbox" name="searchInitiatorList" id="searchInitiatorList_all" value="all" checked="checked">
						                            <label for="searchInitiatorList_all">전체</label>
						                            <li class="chk_click" onclick="javascript:fn_initiatorAll()" >전체 체크</li>
						                        </li>
						                        <c:forEach items="${initiatorList}" var="initiatorList" varStatus="status">
						                            <li><input type="checkbox" name=searchInitiatorList id="searchInitiatorList_${initiatorList.initServerId}" value="${initiatorList.initServerId}" class="searchInitiatorList_Chk">
						                                <label for="searchInitiatorList_${initiatorList.initServerId}">${initiatorList.initServerId}</label></li>
						                        </c:forEach>
						                    </ul>
							...
							<tr>
								...
								<td class="txt_l" colspan="3"> 
									<input type="radio" name="anaType" id="initiator" value="initiator" checked="checked">
									<label for="b2">Initiator ID</label>
									<input type="radio" name="anaType" id="buySide" value="buySide">
