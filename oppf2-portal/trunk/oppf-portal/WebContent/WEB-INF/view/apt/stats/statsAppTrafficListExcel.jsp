...
<%  
    ...
    String filename = java.net.URLEncoder.encode("APP별트래픽통계["+searchTrafficType+"기준]_"+date+".xls","UTF-8");
    //다운로드하고자 한다면 
 ...
 * @Description : App별 트래픽 통계 excel 조회 목록
   ...
   <table border="1">
       ...
       <tr>
           <c:choose>
               ...
               <c:otherwise>
                   <c:forEach var="chartDisplayNames" items="${chartDisplayNames}" varStatus="status">
                       <th scope="col">호출성공(건)</th>
                       <th scope="col">전체응답(초)</th>
                       <th scope="col">외부응답(초)</th>
                       <th scope="col">호출실패(건)</th>
                       <th scope="col">전체응답(초)</th>
                       <th scope="col">외부응답(초)</th>
                   </c:forEach>
               </c:otherwise>
