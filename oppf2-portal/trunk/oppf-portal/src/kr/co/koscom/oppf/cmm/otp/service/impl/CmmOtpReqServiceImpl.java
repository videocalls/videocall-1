...
public class CmmOtpReqServiceImpl implements CmmOtpReqService{
     ...
     */
    @Transactional
    public int saveOtpProfileDev(CmmOtpReqVO cmmOtpReqVO) throws Exception{
    	int result = 0;
    	...
    	//chkCnt가 0이상이면 수정
		if(chkCnt > 0){
			result = cmmOtpReqDAO.updateOtpProfileDev(cmmOtpReqVO);
		}else{
			result = cmmOtpReqDAO.insertOtpProfile(cmmOtpReqVO);
		}
     ...
     */
    @Transactional
    public Map<String, Object> deleteOtpProfileDev(CmmOtpReqVO cmmOtpReqVO) throws Exception{
    	int result = 0;
    	...
    	//chkCnt가 0이상이면 삭제
		if(chkCnt > 0){
			result = cmmOtpReqDAO.deleteOtpProfileDev(cmmOtpReqVO);
		}
