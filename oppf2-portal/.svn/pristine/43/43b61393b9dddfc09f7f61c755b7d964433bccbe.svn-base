...
//    	String result1 = procCronSvcTermsEmail(request, "G016007");
		...
		// (가)동의서 동의 재동의 안내 메일
		procCronCommonTermsEmail(request, "G016010");
...
//    	model.addAttribute("result1", result1);
	...
	public String procCronCommonTermsEmail(HttpServletRequest request, String emailSendType){
		...
		try{
			//금융거래정보 제공 동의 안내 대상 목록
			termsList = cmmEmailSendService.selectCronCommonTermsList();
		}catch(Exception e){
			result = "01";
			log.debug("################금융거래정보 제공 동의 email 발송  ###########################");
			log.debug(" 금융거래정보 제공동의 메일발송 대상 목록 조회 실패\ne="+e);
			log.debug("###############################################################");
		}
		...
		if(termsList != null) {
			for(CmmEmailCronSvcTermsVO termsVO : termsList) {
				...
				cmmEmailSendVO.setReceiverName(termsVO.getCustomerName()); //받는 사람 이름(실제 이메일에 셋팅됩니다.)
				cmmEmailSendVO.setReceiverEmail(termsVO.getCustomerEmail()); //받는 사람 이메일(실제 이메일에 셋팅되고, 이 주소로 이메일이 발송됩니다.)
				cmmEmailSendVO.setReceiverKind("G018001"); //수신자 종류 - G018001:일반사용자, G018002:기업사용자
				cmmEmailSendVO.setReceiverId(termsVO.getCustomerRegno()); //수신자 ID

				cmmEmailSendVO.setSenderKind("G017001"); //발신자 종류 - G017001:Admin, G017002:Operator, G017003:System
				cmmEmailSendVO.setSenderId(termsVO.getCustomerRegno()); //발신자 ID

				cmmEmailSendVO.setSendId(termsVO.getCustomerRegno()); //최초 발신자 ID
				cmmEmailSendVO.setCreateId(termsVO.getCustomerRegno()); //생성자ID
				cmmEmailSendVO.setUpdateId(termsVO.getCustomerRegno()); //수정자ID
				...
				cmmEmailSendVO.setTermsStartDate(termsVO.getTermsStartDate());
				cmmEmailSendVO.setTermsExpireDate(termsVO.getTermsExpireDate());

				String pubCompanyName = "";
				try{
					List<String> pubCompanyList = cmmEmailSendService.selectCronCommonTermsPubCompanyList(termsVO);
					for(String pubCompany : pubCompanyList) {
						if(pubCompanyName.equals("")) {
							pubCompanyName = pubCompany;
						} else {
							pubCompanyName += ", " + pubCompany;
						}
					}
				...
				cmmEmailSendVO.setPubcompanyName(pubCompanyName);

				try{
					//1.이메일 템플릿 조회 -> 2.이메일발송 유형 구분 -> 3.메일 발송 -> (성공시) -> 4.com_email_send_info 로그적재 관련
					CmmEmailSendVO emailResultVO = cmmEmailSendService.cmmEmailSend(cmmEmailSendVO, request);
					if("100".equals(emailResultVO.getMailCode())){ //데이터 연동 성공
...
//    		sql1.append(" date_format(b.terms_expire_date - INTERVAL 1 YEAR, '%Y%m%d') ");
    	...
    	/*
    	//test용
    	//paramVO.setEmailSendType("'G016006'");
    	*/
