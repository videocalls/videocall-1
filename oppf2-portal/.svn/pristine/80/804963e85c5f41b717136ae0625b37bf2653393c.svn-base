...
function fn_sessionIdCheck(){
	...
	if(null != g_customerId && '' != g_customerId){
		$("#loginId").val(g_customerId);
		$(".loginIdChk").attr('readonly', true);
		$("#loginPassword").focus();
	}else{
		$("#loginId").focus();
	}
...
<div class="wrap window_popup">
	...
	<div class="account">
		<div class="tit_info">
			<span><c:out value='${ paramVO.app_name }'/></span>에서 KOSCOM <spring:message code='Globals.domain.spt.name' /> 계정을 사용하려면 로그인하세요.
		</div>
		<div class="otp">
			<label for="aa1">아이디</label>
			<input type="text" name="loginId" id="loginId" onkeydown="javascript:if(event.keyCode == 13) $('#loginPassword').focus();"
				   onkeyup="onMoveFocus(this,'loginId',15);" value="" class="loginIdChk">
			<label for="aa2" class="mt15" id="login_type">비밀번호</label>
		...
		<p class="link">
			<span><a href="/usr/mbrReg/mbrReg.do" target="_blank">회원가입</a> / <a href="/spt/cmm/selectIdFindView.do" target="_blank">아이디·비밀번호 찾기</a></span>
		</p>
	</div>
	<!--// 아이디 입력 -->
	</form>
