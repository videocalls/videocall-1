...
<%--
/**  
 * @Name : statsTrafficApiList.jsp
 * @Description : Api Traffic 통계
...
<%-- 로그인 처리 공통 함수 --%>
function fn_login(){
    var url = "<c:url value='/cpt/myp/stats/statsTrafficApiList.do'/>";
...
$(document).ready(function(){
    ...
    $("#btnSearchClear").click(function(){
        util_moveRequest("CptStatsTrafficVO", "<c:url value='/cpt/myp/stats/statsTrafficApiList.do'/>", "_top");
    });
...
function fn_searchTypeChange_new_second() {
    ...
    // 날짜정보가 없으면 디폴트 날짜 설정
    if(g_searchDateFrom == "" && g_searchDateTo == "") {
        fn_searchDateSet(searchTypeValue);
    } else {
...
function fn_search(){
    ...
    //page setting  
    var url = "<c:url value='/cpt/myp/stats/selectStatsTrafficApiList.ajax'/>";
...
function fn_searchExcel(excelType){
    ...
    util_moveRequest("CptStatsTrafficVO", "<c:url value='/cpt/myp/stats/statsTrafficApiListExcel.do'/>", "EXCEL_FRAME");
...
function fn_changeApiNameList(){
    ...
    //page setting
    var url = "<c:url value='/cpt/myp/stats/changeApiNameList.ajax'/>";
    var param = $("#CptStatsTrafficVO").serialize();
...
function fn_changeApiNameListCallBack(data){
    ...
    fn_makeSearchSubCompanyCodeId(subCompanyList);
}

<%-- API 이름 make --%>
function fn_makeSearchSubCompanyCodeId(list){
    var html = "";
...
<%--<input type="hidden" name="searchStatsType" id="searchStatsType" value="apiName" />--%>
            ...
            <article id="content">
                ...
                <div class="con">
                    ...
                    <table class="tbtype_serch">
                        ...
                        <tr>
                            <th scope="row"><label for="">API 서비스</label></th>
                        ...
                        <tr>
                            ...
                            <td class="txt_l" colspan="3">
                                <div class="radio_img">
                                    <input type="radio" id="searchStatsTypeApiName" name="searchStatsType" value="apiName" checked="checked" /><label for="searchStatsTypeApiName"> API 서비스 </label>
                                    <input type="radio" id="searchStatsTypeSubCompany" name="searchStatsType" value="company" /><label for="searchStatsTypeSubCompany"> 앱 개발자 </label>
                                </div>
                            </td>
                        </tr>
