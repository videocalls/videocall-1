...
public class CptStatsTrafficServiceImpl implements CptStatsTrafficService{
	...
	public Map<String, Object> selectStatsTrafficList(CptStatsTrafficVO cptStatsTrafficVO) throws Exception {
		...
		} else if("company".equals(cptStatsTrafficVO.getSearchStatsType()) && !"Y".equals(cptStatsTrafficVO.getSearchSubCompanyCodeIdAllYn())) {
			// API 서비스 이름 기준
			resultList = cptStatsTrafficDAO.selectStatsTrafficList(cptStatsTrafficVO);
			searchCodeList = this.getChartDisplayNames(cptStatsTrafficVO.getSubCompanyList());
	...
	private void setServiceSearchInfo(CptStatsTrafficVO cptStatsTrafficVO) {
		...
		if("appName".equals(cptStatsTrafficVO.getSearchStatsType()) && !"Y".equals(cptStatsTrafficVO.getSearchPubCompanyCodeIdAllYn())) {
			...
			cptStatsTrafficVO.setSql1(", B.sub_company_code_id AS codeId, B.sub_company_name AS codeName");
			cptStatsTrafficVO.setSql2("GROUP BY A.std_date, B.sub_company_name, B.sub_company_code_id \n ORDER BY B.sub_company_name, A.std_date asc");
		}
	...
	public List<CptStatsTrafficVO> selectStatsTrafficApiNameListForApp(CptStatsTrafficVO cptStatsTrafficVO) throws Exception {
		return cptStatsTrafficDAO.selectStatsTrafficApiNameListForApp(cptStatsTrafficVO);
	}
	...
	public List<CptStatsTrafficVO> selectStatsTrafficSubCompanyListForApi(CptStatsTrafficVO cptStatsTrafficVO) throws Exception {
		return cptStatsTrafficDAO.selectStatsTrafficSubCompanyListForApi(cptStatsTrafficVO);
	}
}
