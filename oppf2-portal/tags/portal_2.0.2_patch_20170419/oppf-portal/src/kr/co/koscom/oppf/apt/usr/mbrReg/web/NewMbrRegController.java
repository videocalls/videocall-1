...
public class NewMbrRegController {
	...
	private static final Logger log = Logger.getLogger(NewMbrRegController.class);
	...
	private String saveNewMbrReg(@ModelAttribute("newMbrRegVO") NewMbrRegVO paramVO, HttpServletRequest request,
		...
		log.info("------------- saveNewMbrReg.ajax START -------------------");
		...
		log.info("------------------------------------------------------------------------------------------------------");
		log.info("paramVO.getCustomerEmailReceiveYn()="+paramVO.getCustomerEmailReceiveYn());
		
		
		int rs = newMbrRegService.saveNewMbrReg(paramVO);
		model.addAttribute("result", rs);

		log.info("------------- saveNewMbrReg.ajax END ---------------------");
		return "jsonView";
	}
