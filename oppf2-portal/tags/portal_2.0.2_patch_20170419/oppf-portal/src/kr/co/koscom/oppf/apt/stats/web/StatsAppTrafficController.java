...
public class StatsAppTrafficController {
    ...
    @RequestMapping(value="/apt/stats/statsAppTrafficChangeAppNameList.ajax",method = {RequestMethod.POST, RequestMethod.GET})
    public String changeAppNameList(@ModelAttribute("StatsTrafficVO") StatsTrafficVO paramVO, HttpServletRequest request, ModelMap model) throws Exception {
        //1.로그인관련
        LoginVO loginVO = (LoginVO) request.getSession().getAttribute("LoginVO");
        if(loginVO == null){
            model.addAttribute("error", "-1");
        ...
        if("N".equals(paramVO.getSearchSubCompanyCodeIdAllYn()) && "Y".equals(paramVO.getSearchAppNameAllYn())) {
            //앱이름
            List<StatsTrafficVO> appNameList = statsTrafficService.selectStatsTrafficAppNameList(paramVO);
            List<String> appIdList = new ArrayList<>();
            for(StatsTrafficVO appNameInfo : appNameList) {
                appIdList.add(appNameInfo.getAppId());
            }
        ...
        //API 서비스 이름
        List<StatsTrafficVO> apiNameList = statsServiceTrafficService.selectStatsTrafficApiNameListForApp(paramVO);
        model.addAttribute("apiNameList", apiNameList);
