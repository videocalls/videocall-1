...
<%  
    ...
    String filename = java.net.URLEncoder.encode("FIX Engine 통계[Request기준]_"+date+".xls","UTF-8");
    //다운로드하고자 한다면 
   ...
   <table border="1">
       ...
       <tr>
           ...
           <c:choose>
               ...
               <c:otherwise>
                   <c:forEach var="chartDisplayNames" items="${chartDisplayNames}" varStatus="status">
                       <th scope="col" colspan="3" style="width:300px;"><c:out value="${chartDisplayNames.displayText}" /></th>
                   </c:forEach>
               </c:otherwise>
           </c:choose>
       </tr>
       <tr>
           <c:choose>
               ...
               <c:otherwise>
                   <c:forEach var="chartDisplayNames" items="${chartDisplayNames}" varStatus="status">
                       <th scope="col" style="width:100px;">총호출수</th>
                       <th scope="col" style="width:100px;">성공</th>
                       <th scope="col" style="width:100px;">실패</th>
                   </c:forEach>
               </c:otherwise>
           </c:choose>
       </tr>
...
%>
                <tr>
                   <td class="txt_l" align="center"><%= data.get("resultDt") %></td>
                       <%
                       ...
                       %>
                           <td class="txt_l" align="right"><%= data.get("totCnt"+dataFieldName) %></td>
                           <td class="txt_l" align="right"><%= data.get("sucCnt"+dataFieldName) %></td>
                           <td class="txt_l" align="right"><%= data.get("failCnt"+dataFieldName) %></td>
