...
$(document).ready(function(){
    ...
    $('#jqxgrid').on('rowclick', function (event) {
        ...
        strCallBack = "${callBakFunc}";
        
        fn_chkCompanyId(data);
...
function fn_chkCompanyId(data){
	
    //로딩 호출
    gfn_setLoading(true);
        
    //page setting  
    var url = "<c:url value='/apt/fix/selectCompanyIdCnt.ajax'/>";

    var objParam = new Object();
    objParam.companyId = data.companyCodeId;
    var callBackFunc = "fn_chkCompanyIdCallBack";
    <%-- 공통 ajax 호출 --%>
    util_ajaxPage(url, objParam, callBackFunc);
	
}

function fn_chkCompanyIdCallBack(data){
    ...
    if(dup == "N"){
    	...
    	if(util_chkReturn(strCallBack, "s") != ""){
    		try{
    	    	if(opener){
    	    		window.opener.eval(strCallBack)(rowData);
    	    	}else{   
    	    		window.parent.eval(strCallBack)(rowData);
    	    	}
    	    	gfn_closeModal();
    	    }catch(e){}
    	}
    }else{
        alert("이미 등록된 기업코드 입니다.");
