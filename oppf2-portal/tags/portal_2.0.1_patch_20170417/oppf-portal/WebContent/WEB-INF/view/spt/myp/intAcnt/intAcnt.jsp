...
function fn_balanceEquityList(data){
    ...
    if('' == $("#companyList option:selected").val()){
        bal_equityListGridColumns =
                [
                    /*{ text: '금융회사', datafield: 'comName', width: '20%', cellsalign: 'center', align: 'center', pinned: true},
                    ...
                    { text: '평가손액(원)', datafield: 'proLoss', width: '120px', cellsalign: 'right', align: 'center', cellsformat: 'n' },
        ...
        if('' == $("#companyList option:selected").val()){
            trans_GridColumns =
                    [
                        ...
                        { text: '잔고(원)', datafield: 'changeAmt', width: '120px', cellsalign: 'right', align: 'center', cellsformat: 'n' },
                        { text: '금액(원)', datafield: 'changeQty', width: '120px', cellsalign: 'right', align: 'center', cellsformat: 'n' },
                        { text: '수량(주)', datafield: 'qty', width: '80px', cellsalign: 'right', align: 'center', cellsformat: 'n' }
                    ];
        }else{
            trans_GridColumns =
...
function fn_failList(data, id){
    ...
    if(failCnt > 0){
        ...
        $.each(failList, function (index, value) {
            ...
            var li = '<li style="width: 100%;">- ' + failData.comName + ' / '+ failData.vtAccNo + ' / ' + jsonMessage.category + ' / ' + jsonMessage.code + ' / ' + jsonMessage.message + ' </li>';
    ...
    if(val == ''){
        <c:forEach var="data" items="${companyMap}" varStatus="status">
            <c:forEach var="account" items="${data.value}" >
                $("#accountList").append("<option value='${account.customerVtaccountNo}'>${account.customerVtaccountNo}</option>");
            </c:forEach>
        </c:forEach>
    }else{
        <c:forEach var="data" items="${companyMap}" varStatus="status">
                            ...
                            <table class="tbtype_form mt20">
                                    ...
                                    <td>
                                        <span class="sel_box1">
                                            <select id="accountList" name="accountList" style="width:200px;">
                                                ...
                                                <c:forEach var="data" items="${companyMap}" varStatus="status">
                                                        <c:forEach var="account" items="${data.value}" >
                                                    <option value="${account.customerVtaccountNo}">${account.customerVtaccountNo}</option>
                                                        </c:forEach>
                                                </c:forEach>
                                            </select>
                                        </span>
                                ...
                                <tr class="transactionSearch" style="display: none">
                                    ...
                                    <td class="txt_l" colspan="3">
                                        <input type="text" id="searchDateFrom" name="searchDateFrom" style="width:120px;" />
                                        &nbsp;
                                        <input type="text" id="searchDateTo" name="searchDateTo" style="width:120px;" />
