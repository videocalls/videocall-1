...
public class StatsServiceTrafficController {
    ...
    @RequestMapping(value="/apt/stats/changeApiServiceNameList.ajax",method = {RequestMethod.POST, RequestMethod.GET})
    private String changeApiServiceNameList(@ModelAttribute("StatsTrafficVO") StatsTrafficVO paramVO, HttpServletRequest request, ModelMap model)throws Exception{
        //1.로그인관련
        LoginVO loginVO = (LoginVO) request.getSession().getAttribute("LoginVO");
        if(loginVO == null){
            model.addAttribute("error", "-1");
        ...
        if(("N".equals(paramVO.getSearchPubCompanyCodeIdAllYn()) && "Y".equals(paramVO.getSearchApiNameAllYn()))) {
            //API 서비스 이름
            List<StatsTrafficVO> apiNameList = statsServiceTrafficService.selectStatsTrafficApiNameList(paramVO);
            List<String> apiIdList = new ArrayList<>();
            for(StatsTrafficVO apiNameInfo : apiNameList) {
                apiIdList.add(apiNameInfo.getApiId());
            }
            paramVO.setSearchApiName(apiIdList);
            paramVO.setSearchApiNameAllYn("N");
        }

        //앱 개발자
        List<StatsTrafficVO> subCompanyList = statsServiceTrafficService.selectStatsTrafficSubCompanyListForApi(paramVO);
        model.addAttribute("subCompanyList", subCompanyList);
