...
//            setMailContent = OppfStringUtil.replace(setMailContent, "++[$GlobalsDomain]++", GlobalsDomain);
        ...
        }else if("G016009".equals(div)){ // 비회원 인증 유효기만 만료 안내 메일
        	
//
        	systemKind = "spt"; //  관리자 포털에서 자동으로 송신 하는 이메일은 서비스 포털로 설정
            GlobalsEmail = OppfProperties.getProperty("Globals.manager.email."+systemKind);
            GlobalsEmailSendId = OppfProperties.getProperty("Globals.system.emailSendId."+systemKind);
            GlobalsDomain = "http://" + OppfProperties.getProperty("Globals.domain."+systemKind);
            GlobalsDomainName = OppfProperties.getProperty("Globals.domain."+systemKind + ".name");        	
//        	
            setAutomailIDEncrypt = OppfProperties.getProperty("Globals.automailAPI.automailIDEncrypt.G016009");

            setMailContent = OppfStringUtil.replace(setMailContent, "++[$GlobalsDomain]++", GlobalsDomain + "/usr/mbrReg/mbrNewReg.do");
            setMailContent = OppfStringUtil.replace(setMailContent, "++[$GlobalsEmail]++", GlobalsEmail);
            setMailContent = OppfStringUtil.replace(setMailContent, "++[$imgUrl]++", GlobalsDomain);
            setMailContent = OppfStringUtil.replace(setMailContent, "++[$userName]++", cmmEmailSendVO.getReceiverName());
            setMailContent = OppfStringUtil.replace(setMailContent, "++[$dateFrom]++", cmmEmailSendVO.getTermsStartDate());
            setMailContent = OppfStringUtil.replace(setMailContent, "++[$dateTo]++", cmmEmailSendVO.getTermsExpireDate());

            //이용중인 앱 정보
            setMailContent = OppfStringUtil.replace(setMailContent, "++[$useApps]++", cmmEmailSendVO.getUseApps());
        ...
        }else if("G016010".equals(div)) { // 가동의서 유효기간 만료 및 재동의 안내
...
//        	
            ...
            setAutomailIDEncrypt = OppfProperties.getProperty("Globals.automailAPI.automailIDEncrypt.G016010");

            setMailContent = OppfStringUtil.replace(setMailContent, "++[$GlobalsDomain]++", GlobalsDomain);
            setMailContent = OppfStringUtil.replace(setMailContent, "++[$GlobalsEmail]++", GlobalsEmail);
            setMailContent = OppfStringUtil.replace(setMailContent, "++[$imgUrl]++", GlobalsDomain);

            setMailContent = OppfStringUtil.replace(setMailContent, "++[$termsStartDate]++", cmmEmailSendVO.getTermsStartDate());
            setMailContent = OppfStringUtil.replace(setMailContent, "++[$termsEndDate]++", cmmEmailSendVO.getTermsExpireDate());
            setMailContent = OppfStringUtil.replace(setMailContent, "++[$termsPubCompanyList]++", cmmEmailSendVO.getPubcompanyName());
    ...
    public List<CmmEmailCronSvcTermsVO> selectCronCommonTermsList() throws Exception{
    	return cmmEmailSendDAO.selectCronCommonTermsList();
    }

    public List<String> selectCronCommonTermsPubCompanyList(CmmEmailCronSvcTermsVO cmmEmailCronSvcTermsVO) throws Exception{
    	return cmmEmailSendDAO.selectCronCommonTermsPubCompanyList(cmmEmailCronSvcTermsVO);
    }
}
