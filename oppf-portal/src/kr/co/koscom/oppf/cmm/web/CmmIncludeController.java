...
public class CmmIncludeController {
    ...
    private String includeLeftSpt(HttpServletRequest request, HttpServletResponse response, ModelMap model)throws Exception{
        ...
        } else if(uri.indexOf("appList") > -1) {
            menuType = "G020002";
            parentMenuId = "06";
            currentMenuId = "06002";
        } else if(uri.indexOf("appDetail") > -1) {
            menuType = "G020002";
            parentMenuId = "06";
            currentMenuId = "06002";
        } else if(uri.indexOf("appAccountSelect") > -1) {
            menuType = "G020002";
            parentMenuId = "06";
            currentMenuId = "06002";
        } else if(uri.indexOf("appCreateResult") > -1) {
            menuType = "G020002";
            parentMenuId = "06";
            currentMenuId = "06002";
        }
    ...
    private String includeLeftCpt(HttpServletRequest request, HttpServletResponse response, ModelMap model)throws Exception{
        ...
        String issueVtaccount = "X";
        if(loginVO != null){
        	issueVtaccount = loginVO.getIssue_vtaccount(); // 기업정보 - 계좌발급주체 Y : 자체발급, N : 코스콤 발급, X : 해당없음
        }        
        ...
        if( uri.indexOf("cptLogin.jsp") > -1 ){ //로그인
            menuType = "G020001"; 
            parentMenuId = "01";
            currentMenuId = "01001";
            ...
            if( "G002001".equals(companyServiceType) ){ //둘다[기업핀테크서비스타입(=G002)(001:둘다,002:서비스제공자,003:앱개발자)]
                // 서비스 제공자 이면서 가상계좌발급 주체이면 동의서 열람 가능
                if (!"X".equals(issueVtaccount))
                	searchMenuTypeList.add("G020006");
            ...
            }else if( "G002002".equals(companyServiceType) ){ //서비스제공자[기업핀테크서비스타입(=G002)(001:둘다,002:서비스제공자,003:앱개발자)]
           	{
                ...
                menuType = "G020005"; //G020:메뉴형태[001:login, 002:menu, 003:mypage, 004:mypage-핀테크, 005:mypage-금투사]
                searchMenuTypeList.add("G020005");
            }
