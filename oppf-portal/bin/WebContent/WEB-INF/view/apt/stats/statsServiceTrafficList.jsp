...
<%--
/**  
 * @Name : statsServiceTrafficList.jsp
 * @Description : 서비스별 Traffic 통계
...
var g_searchDateFrom = "";
var g_searchDateTo = "";
var g_searchTimeFrom = "";
var g_searchTimeTo = "";

var g_gridColumnAll = [
    { text: '일', datafield: 'Date', width: '100px', cellsalign: 'center', align: 'center', pinned: true },
    { text: '호출성공(건)', datafield: 'cntApiDurationY', columngroup:'dataForAll', width: '100px', cellsalign: 'right', align: 'center' },
    { text: '전체응답(초)', datafield: 'apiDurationY', columngroup:'dataForAll', width: '100px', cellsalign: 'right', align: 'center' },
    { text: '외부응답(초)', datafield: 'sifDurationY', columngroup:'dataForAll', width: '100px', cellsalign: 'right', align: 'center' },
    { text: '호출실패(건)', datafield: 'cntApiDurationN', columngroup:'dataForAll', width: '100px', cellsalign: 'right', align: 'center' },
    { text: '전체응답(초)', datafield: 'apiDurationN', columngroup:'dataForAll', width: '100px', cellsalign: 'right', align: 'center' },
    { text: '외부응답(초)', datafield: 'sifDurationN', columngroup:'dataForAll', width: '100px', cellsalign: 'right', align: 'center' }
];
...
<%-- 로그인 처리 공통 함수 --%>
function fn_login(){
    var url = "<c:url value='/apt/stats/statsServiceTrafficList.do'/>";
...
$(document).ready(function(){
    ...
    $("#btnSearchClear").click(function(){
        ...
        $("#searchPubCompanyCodeIdAllYn").val("Y");
        util_moveRequest("StatsTrafficVO", "<c:url value='/apt/stats/statsServiceTrafficList.do'/>", "_top");
    });
    ...
    $("input[name=searchPubCompanyCodeId]").click(function(){
        if(($(this).attr("id") == "searchPubCompanyCodeId_all") || ($("input[name=searchPubCompanyCodeId]:checked").length <= 5)) {
            setTimeout(function(){
    ...
    $("input[name=searchApiName]").click(function(){
        if(($(this).attr("id") == "searchApiName_all") || ($("input[name=searchApiName]:checked").length <= 5)) {
            setTimeout(function(){
...
function fn_searchTypeChange_new_second() {
    // 분,시간 검색의 경우는 time input box show
    var searchTypeValue = $("input[name=searchTrafficType]:checked").val();
    if(searchTypeValue == "min" || searchTypeValue == "hourly"){
        $(".searchTime").show();
    }else{
    ...
    // 날짜정보가 없으면 디폴트 날짜 설정
    if(g_searchDateFrom == "" && g_searchDateTo == "" && g_searchTimeFrom == "" && g_searchTimeTo=="") {
        fn_searchDateSet(searchTypeValue);
    } else {
        $("#searchDateFrom").val(g_searchDateFrom);
        $("#searchDateTo").val(g_searchDateTo);
        $("#searchTimeFrom").val(g_searchTimeFrom);
        $("#searchTimeTo").val(g_searchTimeTo);
    }
    // from date 와 time 은 그대로 가져간다.
    // 각 분류별로 최대기간을 넘어가는 경우 디폴트 처리를 한다.
}

function fn_searchDateSet(searchTypeValue) {
    var searchDateFrom = "";
    ...
    /*g_searchDateFrom = searchDateFrom;
    g_searchDateTo = searchDateTo;
    g_searchTimeFrom = searchTimeFrom;
    g_searchTimeTo = searchTimeTo;*/
}
...
function fn_search(){
    ...
    var pubCompanyList = new Array();
    $("input[name=searchPubCompanyCodeId]:checked").each(function() {
        var code = $(this).val();
        var value = $(this).next('label').text();
        pubCompanyList[pubCompanyList.length] = code + "/" + value;
    });
    $("#pubCompanyList").val(pubCompanyList);
    ...
    //page setting  
    var url = "<c:url value='/apt/stats/selectStatsServiceTrafficList.ajax'/>";
...
//            seriesData[seriesData.length] = {dataField: headers[i].dataField, displayText:headers[i].displayText, color:chartColorGroup[i]};
            ...
            columns.push({ text: '호출성공(건)', datafield: "cntApiDurationY", columngroup:'group'+i, width: '100px', cellsalign: 'right', align: 'center' });
            columns.push({ text: '전체응답(초)', datafield: "apiDurationY", columngroup:'group'+i, width: '100px', cellsalign: 'right', align: 'center' });
            columns.push({ text: '외부응답(초)', datafield: "sifDurationY", columngroup:'group'+i, width: '100px', cellsalign: 'right', align: 'center' });
            columns.push({ text: '호출실패(건)', datafield: "cntApiDurationN", columngroup:'group'+i, width: '100px', cellsalign: 'right', align: 'center' });
            columns.push({ text: '전체응답(초)', datafield: "apiDurationN", columngroup:'group'+i, width: '100px', cellsalign: 'right', align: 'center' });
            columns.push({ text: '외부응답(초)', datafield: "sifDurationN", columngroup:'group'+i, width: '100px', cellsalign: 'right', align: 'center' });
        } else {
            if(headers.length == 1) {
            ...
            columns.push({ text: '호출성공(건)', datafield: 'cntApiDurationY_'+ headers[i].dataField, columngroup:'group'+i, width: '100px', cellsalign: 'right', align: 'center' });
            columns.push({ text: '전체응답(초)', datafield: 'apiDurationY_'+ headers[i].dataField, columngroup:'group'+i, width: '100px', cellsalign: 'right', align: 'center' });
            columns.push({ text: '외부응답(초)', datafield: 'sifDurationY_'+ headers[i].dataField, columngroup:'group'+i, width: '100px', cellsalign: 'right', align: 'center' });
            columns.push({ text: '호출실패(건)', datafield: 'cntApiDurationN_'+ headers[i].dataField, columngroup:'group'+i, width: '100px', cellsalign: 'right', align: 'center' });
            columns.push({ text: '전체응답(초)', datafield: 'apiDurationN_'+ headers[i].dataField, columngroup:'group'+i, width: '100px', cellsalign: 'right', align: 'center' });
            columns.push({ text: '외부응답(초)', datafield: 'sifDurationN_'+ headers[i].dataField, columngroup:'group'+i, width: '100px', cellsalign: 'right', align: 'center' });
        }
    ...
    if(searchTypeValue == "min" || searchTypeValue == "hourly") {
        xAxis = {
            dataField: "dateForChart",
            type: 'date',
            valuesOnTicks: true,
            minValue: '04-04-2017 09:00',
            maxValue: '04-11-2017 08:00',
            baseUnit: 'hour',
...
function fn_searchExcel(excelType){
    ...
    util_moveRequest("StatsTrafficVO", "<c:url value='/apt/stats/statsServiceTrafficListExcel.do'/>", "EXCEL_FRAME");
...
function fn_changePubCompanyList(){
    ...
    //page setting  
    var url = "<c:url value='/apt/stats/changeServiceCompanyList.ajax'/>";
    var param = $("#StatsTrafficVO").serialize();
...
function fn_changeApiNameList(){
    ...
    //page setting
    var url = "<c:url value='/apt/stats/changeApiServiceNameList.ajax'/>";
    var param = $("#StatsTrafficVO").serialize();
...
function fn_changeApiNameListCallBack(data){
    ...
    fn_makeSearchAppName(subCompanyList);
}

<%-- 앱 개발자 make --%>
function fn_makeSearchAppName(list){
    var html = "";
    $("#searchSubCompanyCodeData >").remove();

    html += "<li>";
    html += "<input type='checkbox' name='searchSubCompanyCodeId' id='searchSubCompanyCodeId_all' value='all' checked='checked'>";
    html += "<label for='searchSubCompanyCodeId_all'>전체</label>";
    html += "</li>";

    if(util_chkReturn(list, "s") != ""){
        $.each(list, function(idx, data){
            html += "<li>";
            html += "<input type='checkbox' name='searchSubCompanyCodeId' id='searchSubCompanyCodeId_"+data.companyProfileRegNo+"' value='"+data.companyCodeId+"'>";
            html += "<label for='searchSubCompanyCodeId_"+data.companyProfileRegNo+"'>"+data.companyNameKorAlias+"</label>";
            html += "</li>";
        });
    }
    $("#searchSubCompanyCodeData").append(html);

    //event 셋팅
    gfn_initCheckbox("searchSubCompanyCodeId", 5);
}
</script>
...
<form:form commandName="StatsTrafficVO" name="StatsTrafficVO" method="post">
    ...
    <input type="hidden" name="pubCompanyList" id="pubCompanyList" value="" />
    <input type="hidden" name="apiNameList" id="apiNameList" value="" />
    <input type="hidden" name="subCompanyList" id="subCompanyList" value="" />
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="txt_l" colspan="3">
                                    <!-- chk_list_wrap -->
                                    <div class="chk_list_wrap type2">
                                        <ul id="searchSubCompanyCodeData">
                                            <li>
                            ...
                            <tr>
                                ...
                                <td class="txt_l" colspan="3">
                                    <input type="radio" id="searchStatsTypeCompany" name="searchStatsType" value="pubCompany" checked="checked" /><label for="searchStatsTypeCompany"> 서비스 제공자 </label>
                                    <input type="radio" id="searchStatsTypeServiceName" name="searchStatsType" value="apiName" /><label for="searchStatsTypeServiceName"> API 서비스 이름 </label>
                                    <input type="radio" id="searchStatsTypeSubCompany" name="searchStatsType" value="subCompany" /><label for="searchStatsTypeSubCompany"> 앱 개발자 </label>
