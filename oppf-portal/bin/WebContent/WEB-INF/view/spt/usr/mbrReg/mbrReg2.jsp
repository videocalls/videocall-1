...
<!-- <object classid="CLSID:EC5D5118-9FDE-4A3E-84F3-C2B711740E70" codebase="http://www.signkorea.com/SKCommAX.cab#version=9,9,0,5" id="CertManX" width="0" height="0"></object> -->
...
$(document).ready(function(){
    ...
    $("#btnRegistCert").bind("click", function(e){
        /* if( !fn_SaveValidate1() ){ //유효성검증호출
            return false;
        }
        if( !fn_SaveValidate2() ){ //유효성검증호출
            return false;
        } */
...
//         objParam.customerSsn        = customerSsn;
        ...
        var url = "<c:url value='/usr/mbrReg/saveMbrReg2.ajax'/>";
        $.ajaxSettings.traditional = true; //ajax 배열 parameter 처리 설정 
        $.ajax({
            type    : "post"
           ,contentType: "application/json"
           ...
           ,success : function(data){
               var msgAlert = "<spring:message code='success.alert.regist'/>";
               alert(msgAlert);
               objParam.customerStep = 'G006003'; //회원 가입 Step:G006(001:본인인증, 002:공인인증서등록, 003:약관동의, 004:개인정보입력, 005:이메일인증, 006:임시비밀번호발급, 007:임시비밀번호수정)
               objParam.customerVerify = customerVerify; //회원검증값
               util_movePage("<c:url value='/usr/mbrReg/mbrReg.do'/>",objParam);
           }
           ,error   : function(){
               var msgAlert = "<spring:message code='fail.alert.regist'/>";
            ...
            <article id="content">
                ...
                <div class="con">
                    <div class="step_area">
                        <ul>
                            ...
                            <li class="on"><div>공인인증서 등록</div></li><!-- 현재step -->
                            <li><div>약관동의</div></li>
                    ...
                    <input type="hidden" name="customerDn" id="customerDn" value="<c:out value="${paramVO.customerVerify}" />" />
                ...
                -->
                    ...
                    <div class="btn_area">
                        ...
                        <a href="javascript:void(0);" id="btnRegistCert" class="btn_type1">등록</a>
