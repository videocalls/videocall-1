...
function fn_createCommonTerms(){
  ...
  //page setting  
    var url = "<c:url value='/apt/sptUsr/createCommonTerms.ajax'/>";
    var param = $("#NewMbrRegVO").serialize();
    var callBackFunc = "fn_saveCallBack";
    <%-- 공통 ajax 호출 --%>
    util_ajaxPage(url, param, callBackFunc);
...
function fn_removeCommonTerms() {
    var confirmMsg = "코스콤 범용 정보 제공 동의서를 폐기 하시겠습니까?";
    if( confirm(confirmMsg) ){
        //로딩 호출
        gfn_setLoading(true);
        var url = "<c:url value='/usr/svcAppl/removeCommonTerms.ajax'/>";
        var callBackFunc = "fn_removeCommonTermsCallBack";
        var objParam = new Object();
        objParam.type = "apt";
        objParam.customerRegNo = $("#customerRegNo").val();
        <%-- 공통 ajax 호출 --%>
...
function fn_removeCommonTermsCallBack(data) {
    //로딩 호출
    gfn_setLoading(false);
    ...
    if(data.rsCd == '00'){
        ...
        var objParam = new Object();
        objParam.customerRegNo = $("#customerRegNo").val();
        util_movePage("<c:url value='/apt/sptUsr/sptUserManageDtl.do'/>", objParam);
    } else {
 ...
 * 코스콤 범용 정보 제공 동의 
 */
function fn_CommonTerms() {
    var confirmMsg = "코스콤 범용 정보 제공 동의를 하시겠습니까?";
    if( confirm(confirmMsg) ){
        //로딩 호출
        gfn_setLoading(true);
       var url = "<c:url value='/apt/usr/commonTerms.ajax'/>";
        var callBackFunc = "fn_CommonTermsCallBack";
        var objParam = new Object();
        objParam.type = "apt";
        objParam.customerRegNo = $("#customerRegNo").val();
        <%-- 공통 ajax 호출 --%>
...
/**
 * 코스콤 범용 정보 제공 동의 CallBack
 */
function fn_CommonTermsCallBack(data) {
    //로딩 호출
    gfn_setLoading(false);
					...
					<div class="tb_write1">
                        ...
                        --%>
								...
								<tr>
		                            ...
		                            <td class="txt_l" >${resultDetail.temporaryMemberName}</td>
		                            <th scope="row">가입 구분</th>
		                            <td class="txt_l">${resultDetail.customerJoinTypeName}</td>
		                            </td>                 
		                        </tr>
								<tr>
									<th scope="row" colspan="2">계정 상태</th>
									<td class="txt_l"><select
		                            ...
		                            <td class="txt_l"><input type="text" name="deviceId" id="deviceId" style="width: 238px;" maxlength="200" value="${deviceDetail.deviceUniqueId}" /></td>
								</tr>
								<tr>
								...
								<tr>
									<th scope="row" colspan="2">코스콤 범용 정보 제공 동의</th>
									<td class="txt_l">
											<c:if test="${resultDetail.commonTermsYn eq 'Y' }"> 동의 <span class="btn_gray1"><a href="javascript:fn_commonTermsPopup();">동의서보기</a></span> <span class="btn_gray1"><a href="javascript:fn_removeCommonTerms();">동의서폐기</a> ( 약관번호 : <c:out value='${resultDetail.commonTermRegNo}'/> )</span></c:if>
											<c:if test="${resultDetail.commonTermsYn eq 'N' }"> 미동의 <span class="btn_gray1"><a href="javascript:fn_CommonTerms();">동의</a></span></c:if>
									</td>
									<th scope="row">등록 일시</th>
									<td class="txt_l"><c:out
											value='${resultDetail.commonTermsChgDate}' /></td>
								</tr>
								...
								<tr>
									...
									<select id="customerOtpYn" name="customerOtpYn" style="min-width: 80px;">
									 ...
									 <c:if test="${resultDetail.customerOtpYn eq 'Y' }"> 
									<input type="text" style="width: 238px;" maxlength="200" readonly="readonly" value="${resultDetail.customerOtpId}" />
									</c:if>
									</td>
									<th scope="row">등록 일시</th>
