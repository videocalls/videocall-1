...
public class SvcApplController {
    ...
    private String registAsumAcntIsuPopup(@ModelAttribute("ComCompanyProfileVO") ComCompanyProfileVO paramVO, HttpServletRequest request, ModelMap model)throws Exception{
        ...
        //6.금투사 약관 정보 조회 
        log.debug("약관 정보 조회 ");
        SptCustomerCompanyTermsProfileVO sptCustomerCompanyTermsProfileVO = svcApplService.selectSvcMemberCompanyTermsConsntList(pSvcApplVO);
        if(sptCustomerCompanyTermsProfileVO != null){
            model.addAttribute("returnCode", "Y");
            model.addAttribute("sptCustomerCompanyTermsProfileVO", sptCustomerCompanyTermsProfileVO);
        } else {
            model.addAttribute("returnCode", "N");
        }
        
        
        model.addAttribute("sptCustomerCompanyTermsProfileVO", sptCustomerCompanyTermsProfileVO);
        log.debug("sptCustomerCompanyTermsProfileVO {}",sptCustomerCompanyTermsProfileVO);
        //7.oauthToken정보 취득
        ComOauthTokenVO pComOauthTokenVO = new ComOauthTokenVO();
        pComOauthTokenVO.setCustomerRegNo(customerRegNo);
        ComOauthTokenVO rsComOauthTokenVO = comOauthTokenService.selectComOauthTokenInfo(pComOauthTokenVO);
...
//        model.addAttribute("rsComCompanyCodeVO", rsComCompanyCodeVO);
     ...
     * @throws             : Exception
     */
	@RequestMapping(value="/usr/svcAppl/saveMemberCompanyTermsConsnt.ajax",method = {RequestMethod.POST, RequestMethod.GET})
	private String saveMemberCompanyTermsConsnt(@RequestBody SptCustomerCompanyTermsProfileVO sptCustomerCompanyTermsProfileVO, HttpServletRequest request, ModelMap model)throws Exception{
		//1.로그인관련
        LoginVO loginVO = (LoginVO) request.getSession().getAttribute("LoginVO");
        if(loginVO == null){
        	model.addAttribute("error", "-1");
        ...
        //사용자 정보 셋팅
        sptCustomerCompanyTermsProfileVO.setCustomerRegNo(customerRegNo);
        sptCustomerCompanyTermsProfileVO.setCreateId(customerRegNo);
                
        int result = svcApplService.saveMemberCompanyTermsConsnt(sptCustomerCompanyTermsProfileVO);
        model.addAttribute("result", result);
