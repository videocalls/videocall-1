...
var appId = '${paramVO.appId}';
var subCompanyCodeId = '${paramVO.subCompanyCodeId}';
...
//                fn_ajaxCallDeleteFolderFile(); //TSA파일삭제처리
                ...
                objParam.termsAuthType = "G032001";
                objParam.termsCreatedYn = "N";
                objParam.termsRegNo = "";
                fn_openerCallBackSend(objParam); //부모창콜백처리
...
/* TSA응답결과 openr전송 함수 */
function fn_openerCallBackSend(objParam){
	...
	if(util_chkReturn(strCallBack, "s") != ""){
		if(opener){
			if(typeof window.opener.eval(strCallBack) === 'function'){
				window.opener.eval(strCallBack)(objParam);
				window.close();
			}
		}else{
			if(typeof window.parent.eval(strCallBack) === 'function'){
				window.parent.eval(strCallBack)(objParam);
...
function fn_openArsTerms(){
    ...
    var objParam = new Object();
    objParam.customerRegNo  = customerRegNo;
    objParam.customerName  = "${appTermsInfo.customerName}";
    objParam.termsEndDate = "${appTermsInfo.termsEndDate}";
    //ARS인증 정보
    objParam.customerPhone    = "${resultCustomerInfoVO.customerPhone}";
    //핀테크기업코드명
    objParam.subcompanyName    = "${appTermsInfo.subCompanyName}";
    var rsPubcompanyList = '';
    <c:forEach items="${appTermsInfo.pubCompanyList}" var="item" varStatus="status">
        rsPubcompanyList += "${item}";
...
function fn_openArsTermsCallBack(objParam){
//    this.rsCd = rsCd;
//    this.rsMsg = rsMsg;
    //ARS 인증 모달 창 종료를 위해 타임아웃 설정
//    setTimeout(function(){ fn_openerCallBackSend(objParam); }, 1000);
    fn_openerCallBackSend(objParam);
}
...
function fn_termsAgreeNoticeOk() {
    if(bntClickType === "sign") {
        certsign();
    } else if(bntClickType === "ars") {
        fn_openArsTerms();
    }
...
$(document).ready(function(){
    ...
    $("#btnCert").bind("click", function(e){
        //util_movePage('imsi');
        <c:choose>
            <c:when test="${beforeAppTerms ne null}">
                bntClickType = "sign";
    ...
    $("#btnVerify").click(function(){
        <c:choose>
            <c:when test="${appTermsInfo.termsAuthType eq 'G032001'}" >
            //공인인증서 검증
            certsignVerify();
            </c:when>
            <c:otherwise>
					...
					<div style="font-weight:bold; line-height:20px; padding:15px; margin-top:10px; border:1px solid #9e9d9d;">
						<ol style="font-size:12px; list-style:none; margin:0; padding:0;">
							<li style="margin-bottom:5px;">1. 정보를 제공받을 기업명 :
                                <span style="color:#015dab;">
                                    <c:out value='${appTermsInfo.subCompanyName}'/>(<c:out value='${appTermsInfo.subCompanyCodeId}'/>)
                                </span></li>
							<li style="margin-bottom:5px;">2. 정보제공의 범위와 사용목적
								...
								<p style="margin:0; padding:0;">O 사용목적 : 핀테크 서비스를 통한 금융서비스 제공</p>
							</li>
                            <li style="margin-bottom:5px;">3. 유효기간 :
                                <span style="color:#015dab;"><c:out value='${appTermsInfo.termsStartDate}'/></span> ~
                                <span style="color:#015dab;"><c:out value='${appTermsInfo.termsEndDate}'/></span>
								...
								<table cellspacing="0" cellpadding="0" border="0" style="width:100%; margin:5px 0; border-top:1px solid #d3d2d2; border-right:1px solid #d3d2d2;">
									...
									<tr>
										...
										<td style="padding:6px 5px; font-size:12px; font-weight:bold; color:#001a30; border-bottom:1px solid #d3d2d2;">
										    <c:out value='${appTermsInfo.customerName}'/>
										</td>
										<th scope="row" style="padding:3px 5px; font-size:12px; border-right:1px solid #d3d2d2; border-left:1px solid #d3d2d2; border-bottom:1px solid #d3d2d2; background:#f3f2f2;">
											생년월일
										</th>
										<td style="padding:6px 5px; font-size:12px; font-weight:bold; color:#001a30; border-bottom:1px solid #d3d2d2;">
                                            <c:out value='${appTermsInfo.customerBirthDay}'/>
										</td>
                                        <th scope="row" style="padding:3px 5px; font-size:12px; border-right:1px solid #d3d2d2; border-left:1px solid #d3d2d2; border-bottom:1px solid #d3d2d2; background:#f3f2f2;">
                                            휴대폰
                                        </th>
                                        <td style="padding:6px 5px; font-size:12px; font-weight:bold; color:#001a30; border-bottom:1px solid #d3d2d2;">
                                            <c:out value='${appTermsInfo.customerPhone}'/>
                                        </td>
                                        <th scope="row" style="padding:3px 5px; font-size:12px; border-right:1px solid #d3d2d2; border-left:1px solid #d3d2d2; border-bottom:1px solid #d3d2d2; background:#f3f2f2;">
										    이메일
										</th>
										<td style="padding:6px 5px; font-size:12px; font-weight:bold; color:#001a30; border-bottom:1px solid #d3d2d2;">
										    <c:out value='${appTermsInfo.customerEmail}'/>
										</td>
									</tr>
									</tbody>
								</table>
							...
							<li style="margin-bottom:5px;">5. 정보제공 금융투자회사
								...
								<c:forEach var="pcList" items="${appTermsInfo.pubCompanyList}" varStatus="status">
                                    <span>O <c:out value='${pcList}'/>  </span>
								</c:forEach>
								</div>
							</li>
						...
						<div style="font-size:12px; color:#1b1b1b; margin:10px -15px -15px; padding:10px 20px; background:#f8f8f8;">
							...
							<div style="font-size:13px; font-weight:bold; text-align:center;">
								<span style="color:#015dab;"><c:out value='${appTermsInfo.termsCreateDate}'/></span>
							</div>
		              ...
		              <c:if test="${termsAuthYn eq 'Y' || termsAuthYn eq 'R'}">
			            ...
			            <div class="btn_area" style="margin-top:20px;">
                            ...
                            <%--<c:if test="${devMode eq 'prd'}">
                                <a href="javascript:void(0);" id="btnArsCert" class="btn_type5" style="background: rgba(176, 38, 1, 0.48);pointer-events: none;cursor: default;">ARS 인증</a>
                            </c:if>--%>
                            <c:if test="${arsUseYn eq 'Y' and arsUseCount >= fn:length(appTermsInfo.pubCompanyList)}">
						               ...
						               <table cellspacing="0" cellpadding="0" border="0" style="width:100%; margin:5px 0; border-top:1px solid #d3d2d2; border-right:1px solid #d3d2d2;">
	                                    ...
	                                    <tr>
	                                        ...
	                                        <td style="padding:6px 14px; font-size:12px; font-weight:bold; color:#001a30; border-bottom:1px solid #d3d2d2; border-right:1px solid #d3d2d2;">
                                                <c:choose>
                                                    <c:when test="${appTermsInfo.termsAuthType eq 'G032001'}" >
                                                        <c:out value='${appTermsInfo.customerSignDn}'/>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <c:out value='${appTermsInfo.termsCreateDate}'/>
                                                    </c:otherwise>
                                                </c:choose>
	                                        </td>
	                                        <td rowspan="3" style="padding:14px 14px; font-size:12px; font-weight:bold; color:#001a30; border-bottom:1px solid #d3d2d2;">
	                                            ...
	                                            <c:choose>
	                                                <%-- 폐기 --%>
										            <c:when test="${appTermsInfo.termsStatus eq 'G030003'}" >
	                                    ...
	                                    <tr>
	                                        ...
	                                        <td style="padding:6px 14px; font-size:12px; font-weight:bold; color:#001a30; border-bottom:1px solid #d3d2d2; border-right:1px solid #d3d2d2;">
                                                <c:choose>
                                                    <c:when test="${appTermsInfo.termsAuthType eq 'G032001'}" >
                                                        <c:set var="customerSignData" value="${fn:substring(appTermsInfo.customerSignData, 1, 60)}" />
                                                        <c:out value='${customerSignData}'/>
                                                        <c:if test="${ not empty customerSignData }">...</c:if>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <c:out value='${appTermsInfo.customerSignData}'/>
                                                    </c:otherwise>
                                                </c:choose>
	                                            <input type="hidden" id="customerSignData" value="${ appTermsInfo.customerSignData }" />
	                                        </td>
	                                    </tr>
	                                    <tr>
	                                        <th scope="row" style="padding:8px 12px; font-size:12px; border-right:1px solid #d3d2d2; border-left:1px solid #d3d2d2; border-bottom:1px solid #d3d2d2; background:#f3f2f2;">
	                                        ...
	                                        <td style="padding:6px 14px; font-size:12px; font-weight:bold; color:#001a30; border-bottom:1px solid #d3d2d2; border-right:1px solid #d3d2d2;">
	                                            <c:out value='${appTermsInfo.customerTsaData}'/>
	                                        </td>
	                                    </tr>
	                                    </tbody>
	                                </table>
...
<input type="hidden" name="termsRegNo" id="termsRegNo" value="${appTermsInfo.termsRegNo}"/>
</form:form>
<%-- 다운용 frame --%>
                ...
                <div class="agree_box re_box">
                    <ol>
                        <li>1. 정보를 제공받을 기관명 : <br>
                            <span class="point04">
                                <c:if test="${beforeAppTerms ne null}">
                                    <c:out value='${beforeAppTerms.subCompanyName}'/>(<c:out value='${beforeAppTerms.subCompanyCodeId}'/>)
                                </c:if>
                            </span>
                        </li>
                        <li>2. 정보제공 금융투자회사<br>
                            <div class="gray_box">
                                <c:if test="${beforeAppTerms ne null}">
                                    <c:forEach var="pubCompany" items="${beforeAppTerms.pubCompanyList}">
                                        <span class="point04">○ <c:out value='${pubCompany}'/></span>
                                    </c:forEach>
                                </c:if>
                            </div>
                        </li>
                        <c:if test="${beforeAppTerms ne null}">
                            <li>3. 유효기간 : <br><span class="point04"><c:out value='${beforeAppTerms.termsStartDate}'/></span> ~ <span class="point04"><c:out value='${beforeAppTerms.termsEndDate}'/></span></li>
                        </c:if>
                    </ol>
                </div>
