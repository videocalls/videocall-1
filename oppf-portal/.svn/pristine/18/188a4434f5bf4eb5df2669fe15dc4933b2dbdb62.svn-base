...
function fn_InfoChangeConfrm(){
    ...
    if(customerPassword1 != "" || customerPassword2 != ""){
        ...
        if( idValuePw(userId, customerPassword1)){
...
function fn_moveList(resultDetail){
	alert("개인 정보가 변경되어 로그아웃됩니다. 다시 로그인해주세요.");
	var customerPassword1 = $("#customerPassword1").val(); //비밀번호
    var customerPassword2 = $("#customerPassword2").val(); //비밀번호확인
	var objParam = new Object();
	//(6/19 이민우 상무님)정보 수정 경우 '개인 정보 변경에 따라 로그아웃 하겠습니다. 다시 로근인 하여 주십시오' 등으로 alert 띄우고, 비밀 번호 변경은 로그인 페이지 기타 변경은 마이페이지로 이동이 맞음
...
$(document).ready(function(){
    ...
    $('input[type=radio][name=customerOtp]').change(function() {
		...
		if(saveCustomerOtpYn == "Y"){
            ...
            //해제 시 -> 폐기
                ...
                $('input:checkbox[name="reRegOtpYn"]').is(":checked") ==  false;
	        }
...
/*
            	...
            	$('input[type=radio][name=customerOtp]')
  */          	
				$(':radio[name="customerOtp"]:checked').val();
            	$('input:radio[name=customerOtp]:input[value=Y]').attr("checked", true);
...
// 	fn_setValemail(customerEmail);
    ...
    var emailCheck = "<c:out value="${paramVO.customerEmailReceiveYn}" />";
    ...
    /* 이름 변경버튼 클릭시 호출 */
    $("#changeUserInfo").bind("click", function(e){
        fn_mobileCheckAuth();
    });
    
    $("#changeMobileInfo").bind("click", function(e){
    	fn_mobileCheckAuth();
    });
...
function checkCi(sex, birthDay, smobileNo, sname, ci) {
	fn_updateMemberInfo(sex, birthDay, smobileNo, sname, ci);
}


/**
*	본인 인증 완료 후 CI를 통한 회원 정보 수정(이름, 휴대폰 번호)
**/
//function checkCi() {
function fn_updateMemberInfo(sex, birthDay, smobileNo, sname, ci) {
	
	
	
   if(util_chkReturn(smobileNo, "s") == ""){
        alert('휴대폰번호가 없습니다.');
        return false;
    }
    var customerPhone1 = '';
    var customerPhone2 = '';
    var customerPhone3 = '';
     var smobileNo_arr = smobileNo.split("-");
     if(smobileNo_arr.length == 3){
         customerPhone1 = smobileNo_arr[0];
         customerPhone2 = smobileNo_arr[1];
         customerPhone3 = smobileNo_arr[2];
     }else{
         if(smobileNo.length == 10){ //ex)0104568901
             customerPhone1 = smobileNo.substring(0,3);
             customerPhone2 = smobileNo.substring(3,6);
             customerPhone3 = smobileNo.substring(6);
         }else if(smobileNo.length == 11){//ex)01045678901
             customerPhone1 = smobileNo.substring(0,3);
             customerPhone2 = smobileNo.substring(3,7);
             customerPhone3 = smobileNo.substring(7);
         }
     }

 	$("#customerAuthPhone").val(customerPhone1+"-"+customerPhone2+"-"+customerPhone3);
 	$("#customerAuthNameKor").val(sname);
 	$("#customerAuthVerify").val(ci);
 	
	//로딩 호출
	gfn_setLoading(true);
		
	//page setting  
    var url = "<c:url value='/usr/mbrReg/chkMbrCiChkForUpdate.ajax'/>";
    var param = $("#MbrRegVO").serialize();
    var callBackFunc = "fn_updateMemberInfoCallBack";
    <%-- 공통 ajax 호출 --%>
    util_ajaxPage(url, param, callBackFunc);
    
}

function fn_updateMemberInfoCallBack(data){
	//로그인 처리
	if(data.error == -1){
		fn_login();
	    return;
...
<form:form commandName="MbrRegVO" name="MbrRegVO">
	<input type="hidden" name="customerAuthNameKor" id="customerAuthNameKor" value="" />
	<input type="hidden" name="customerAuthVerify" id="customerAuthVerify" value="" />
	<input type="hidden" name="customerAuthPhone" id="customerAuthPhone" value="" />
</form:form>
                    ...
                    <table class="tbtype_form">
                        ...
                        <tbody>
                            <tr>
                                <th scope="row">이름</th>
                                <td>
	                                <c:out value="${paramVO.customerNameKor}" />&nbsp<a href="javascript:void(0);" id="changeUserInfo" class="btn_type3">변경</a>
                            ...
                            <tr>
                                ...
                                <td><c:out value="${paramVO.customerBirthDay}"/></td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="info_bottom">
										...
										<input type="checkbox" id="email_chk" name="email_chk"><label for="email_chk" class="chk_box">수신 동의 (뉴스/이벤트 등)</label>
									</div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <span class="sel_box1">                                        
                                        <c:out value="${paramVO.customerPhone}" />&nbsp<a href="javascript:void(0);" id="changeMobileInfo" class="btn_type3">변경</a>
                                ...
                                <c:when test="${paramVO.customerOtpYn eq 'N'}" >
                                    <tr>
                                        ...
                                        <td>                                
											...
											<!-- 스마트 OTP사용 modify -->
											<ul class="list_radio smart_otp">
												<li>
													<input type="radio" id="Y" name="customerOtp" value="Y">
													<label for="Y"><span>ON</span></label>
												</li>
												<li>
													<input type="radio" id="N" name="customerOtp" value="N" checked="">
													<label for="N"><span>OFF</span></label>
												</li>
											</ul>      
                                            <div class="mt10">
                                ...
                                <c:otherwise>
                                    <tr>
                                        ...
                                        <td>           
											...
											<ul class="list_radio smart_otp">
												<li>
													<input type="radio" id="Y" name="customerOtp" value="Y" checked="">
													<label for="Y"><span>ON</span></label>
												</li>
												<li>
													<input type="radio" id="N" name="customerOtp" value="N">
													<label for="N"><span>OFF</span></label>
												</li>
											</ul><span class="otp_check">
											(&nbsp;<input type="checkbox" id="reRegOtpYn" name="reRegOtpYn"><label for="customerOtpYn" class="chk_box">등록번호 재등록</label>&nbsp;)
											</span>
                                            <div class="info_bottom">
