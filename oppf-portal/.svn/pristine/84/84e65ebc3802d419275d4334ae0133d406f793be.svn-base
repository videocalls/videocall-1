...
public class FixQueueManageController {
    ...
    private String updateFixQueueManageList(@ModelAttribute("FixQueueManageVO") FixQueueManageVO paramVO, HttpServletRequest request, ModelMap model)throws Exception{
        ...
        String port = OppfProperties.
                getProperty("Globals.fix.initiator.port");
        ...
        fixManageService.sendRestTemplate("http://" + paramVO.getInitServerIp() + port + restUrl, HttpMethod.PUT, payload);

        /* 삭제 후 수정 API */
        paramVO.setActiveQueueYn("Y");
        paramVO.setInitServerId(paramVO.getAfterInitServerId());
        payload = queueUpdate(paramVO, loginVO);
        fixManageService.sendRestTemplate("http://" + paramVO.getAfterInitServerIp() + port + restUrl, HttpMethod.PUT, payload);
