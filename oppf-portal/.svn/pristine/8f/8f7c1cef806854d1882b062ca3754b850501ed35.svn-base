...
//     	util_movePage("<c:url value='/usr/mbrReg/mbrReg.do'/>");
    	fn_moveHttpUrl('/usr/mbrReg/mbrNewReg.do');
    });
...
function fn_loginCheck_callBack(data){
	...
	if(loginVO == null){
		...
		var failCount = "<spring:message code='Globals.user.policy.password.failCnt'/>";
        if(failCount <= loginVO.customer_password_fail_cnt){
        	//사용자 정지 상태
		...
		if(loginVO.customer_password_yn == "Y"){
	        ...
	        } else  if(null != data.checkedCommonTerms) {
...
function fn_closeCommonTermsPopup() {
	//returnListURL이 있는 경우
	if(util_chkReturn($("#returnListURL").val(), "s") != "") {
		var url = $("#returnListURL").val();
		var param = new Object();
