 ...
 * @Project  : 코스콤 오픈플렛폼
 * @FileName : CmmMultipartResolver.java
 ...
 * ----------- ------  ----------
 * 2016.04.28  이환덕        최초생성
 * 2011.04.29  이환덕        스프링 3.0 업그레이드 API변경으로인한 수정
 *
 * </pre>
 */
@Slf4j
public class CmmMultipartResolver extends CommonsMultipartResolver {

    public CmmMultipartResolver() {}
     ...
     */
    public CmmMultipartResolver(ServletContext servletContext) {
	...
	for (Iterator<?> it = fileItems.iterator(); it.hasNext();) {
		...
		if (fileItem.getSize() > 0) {
			...
			log.debug("Found multipart file [" + file.getName() + "] of size " + file.getSize() + " bytes with original filename ["
